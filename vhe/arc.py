...
RESUME = True
RESUME_FROM = 87000
RESUME_PATH = "./model/VHE_epoch_" + str(RESUME_FROM) + ".p"

if __name__ == '__main__':
	vhe = VHE(encoder=[Qc(), Qz()],
		  decoder=Px())
	if RESUME: vhe.load_state_dict(torch.load(RESUME_PATH))
	vhe = vhe.cuda()
	print("created vhe")
	...
	optimiser = optim.Adam(vhe.parameters(), lr=1e-5, weight_decay = 1e-5)
	scheduler = lr_scheduler.StepLR(optimiser, step_size=1, gamma=args.lr_decay)

	total_iter = 0
	start = RESUME_FROM if RESUME else 1
	for epoch in range(start, args.max_epochs + 1):
		...
		for batch in data_loader:
			...
			(-score).backward() 
			torch.nn.utils.clip_grad_norm_(vhe.parameters(), 1.0)
			optimiser.step()
			batchnum += 1
