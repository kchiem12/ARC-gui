...
class DataLoader():
    def __init__(self, data, batch_size, labels, k_shot, transforms=[]):
        self.data = data # 就是我们的给在 DataLoader 中给入的 data
    ...
    def next(self):
        ...
        x = self.select_data(x_idx) # 我们给入的 data 的 x_idx 这一部分
        ...
        # target 原始输入，即我们最终想要输出的东西
        # inputs 根据 class 分类的输入，即我们喂给模型(decoder?)的输入
        # sizes 每个 class 中有多少个sample
